...
initStruct (void){
  ...
  if (err = rt_mutex_create (&mutexMessage, NULL))
    {
      rt_printf ("Error mutex create: %s\n", strerror (-err));
      exit (EXIT_FAILURE);
    }
  ...
  if (err = rt_sem_create (&semRechargeWat, NULL, 0, S_FIFO))
    {
      rt_printf ("Error semaphore create: %s\n", strerror (-err));
      exit (EXIT_FAILURE);
    }
  ...
  if (err =
      rt_queue_create (&queueMsgGUI, "toto",
		       MSG_QUEUE_SIZE * sizeof (DMessage), MSG_QUEUE_SIZE,
		       Q_FIFO))
    {
